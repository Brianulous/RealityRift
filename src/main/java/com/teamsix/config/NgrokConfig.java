...
public class NgrokConfig {
	...
	public String ngrokUrl() {
		// consider to use production, dev and test profiles to switch the url string
		// bean
		...
		try {
			URL url = new URL("http://localhost:4040/api/tunnels"); // consider move this to config/property file
			...
			return jsonElement.getAsJsonObject().get("tunnels").getAsJsonArray().get(0).getAsJsonObject()
